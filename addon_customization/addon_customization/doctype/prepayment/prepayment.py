...
def create_prepayment(doc,month):
    #Simple function to create the journal entry of a prepayment for a period
    existing_jes = frappe.get_all("Journal Entry",{"prepayment":doc})
    if existing_jes:
        sample_doc = frappe.get_doc("Journal Entry",existing_jes[0].name)
        new_journal = frappe.copy_doc(sample_doc)
        new_journal.workflow_state = 'draft'
        new_journal.posting_date = frappe.utils.get_datetime(month)
        new_journal.save()
        new_journal.submit()
