	...
	for (company, item, warehouse) in sorted(iwb_map):
		if item_map.get(item):
			...
			if filters.get('price_list'):
				...
				get_val = frappe.get_value("Item Price", {"item_code" : item, "price_list" : filters.get('price_list')}, "name")
				if get_val :
					get_ip = frappe.get_doc("Item Price", get_val)
					additional_data["price_list_rate"] = get_ip.price_list_rate

					additional_data["price_list_amount"] = qty_dict.bal_qty * get_ip.price_list_rate
...
def get_columns(filters):
	...
	columns = [
		...
		{"label": _("Item SubGroup"), "fieldname": "item_subgroup", "fieldtype": "Link", "options": "Item SubGroup", "width": 100},
	...
	if filters.get('price_list'):
		columns += [{"label": _("Price List Rate"), "fieldname": "price_list_rate", "fieldtype": "Currency", "width": 100}]
		columns += [{"label": _("Price List Amount"), "fieldname": "price_list_amount", "fieldtype": "Currency", "width": 150}]
...
def get_items(filters):
	...
	if filters.get("item_subgroup"):
		conditions.append("item.item_subgroup=%(item_subgroup)s")
