...
class PartyLedgerSummaryReport(object):
	...
	def get_data(self):
		...
		invoice_dr_or_cr = "debit" if self.filters.party_type == "Sundry Debtor" else "credit"
		reverse_dr_or_cr = "credit" if self.filters.party_type == "Sundry Debtor" else "debit"
	...
	def get_gl_entries(self):
		...
		join = join_field = ""
		if self.filters.party_type == "Sundry Debtor":
			join_field = ", p.full_name as party_name"
			join = "left join `tabSundry Debtor` p on gle.party = p.name"
		quote = ("""
			select
				gle.posting_date, gle.account,gle.party, gle.voucher_type, gle.voucher_no, gle.against_voucher_type,
		...
		""".format(join=join, join_field=join_field, conditions=conditions), self.filters)
